...
function checkUserRegistration(userId) {
  ...
  // CORSプリフライト（事前検証）を回避するため text/plain で送信
  fetch(GAS_API_URL, {
    method: 'POST',
    headers: {
...
function toggleFaq(element) {
    ...
    // 1. すべてのFAQの回答を閉じ、アイコンを'down'にリセット
...
/**
 * 日付文字列（2026/10/03~04 や 2026/09/12 など）から「本日」「今週末」等のバッジHTMLを生成する
 */
